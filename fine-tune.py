...
class Text2TextModel:
    ...
    # def compute_backtranslation_loss(self, sentences, key_lang, intermediate_lang):
    #     return self.backtranslate(sentences, key_lang, intermediate_lang, num_epochs=1, lr=None, batch_size=5, testing=True)
    ...
    def backtranslate(self, sentences, key_lang, intermediate_lang, num_epochs=1, lr=0.005, batch_size=5, testing = False):
        """
        ...
        for batch_idx in range(num_batches):
            ...
            for epoch in range(num_epochs):
                ...
                print(f"Batch size: {this_batch_size}")
                if epoch == 0 or epoch == num_epochs - 1 or epoch % 10 == 0:
                    self.t2t_model.train(False)
                    intermediate = self.t2t_model.predict(sentences[start_idx:end_idx], source_lang=key_lang, target_lang=intermediate_lang)
                    print(f"Intermediate: {intermediate}")
                    output = self.t2t_model.predict(intermediate, source_lang=intermediate_lang, target_lang=key_lang)
                    print(f"Output: {output}")
                    self.t2t_model.train(True)
                    with open("translationsv3.txt", "a") as f:
...
if __name__ == "__main__":
    ...
    epoch_losses = model.backtranslate(data, key_lang="eng_Latn", intermediate_lang="tel_Telu", num_epochs=20, lr=0.006, batch_size=5)
    # Export epoch losses to a separate file
    losses_df = pd.DataFrame({"epoch_losses": epoch_losses})
    losses_df.to_csv("epoch_losses.csv", index=True)
    # translations = pd.DataFrame({"translations":model.predict(data, source_lang="tel_Telu", target_lang="eng_Latn")})
    # print(translations)

def evaluate_bleu(model: Text2TextModel, ):
    pass
